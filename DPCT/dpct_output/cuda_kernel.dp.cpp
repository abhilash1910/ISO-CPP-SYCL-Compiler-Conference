...
// CHECK-NEXT:}
void hello(const sycl::nd_item<3> &item_ct1) {
  ...
  int num = dpct::max(
      (unsigned int)((item_ct1.get_group(2) + 1) * item_ct1.get_local_range(2) +
                     item_ct1.get_local_id(2)),
      sycl::min(tmp, 45));
  sycl::log((double)(item_ct1.get_group(2) + item_ct1.get_local_range(2) +
                     item_ct1.get_local_id(2)));
  sycl::log((double)(item_ct1.get_local_range(2) + item_ct1.get_local_range(1) +
...
int main(int argc, char **argv) {
  ...
  d_array = (float *)dpct::dpct_malloc(sizeof(float) * size);

  // CHECK: dpct::dpct_memset(d_array, 0, sizeof(float) * size);
  dpct::dpct_memset(d_array, 0, sizeof(float) * size);
  ...
  dpct::get_out_of_order_queue().submit([&](sycl::handler &cgh) {
    auto d_array_acc_ct0 = dpct::get_access(d_array, cgh);

    cgh.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, size / 64) *
                                           sycl::range<3>(1, 1, 64),
                                       sycl::range<3>(1, 1, 64)),
                     [=](sycl::nd_item<3> item_ct1) {
                       simple_kernel((float *)(&d_array_acc_ct0[0]), item_ct1);
                     });
  });

  // CHECK:  dpct::dpct_memcpy(h_array, d_array, 360 * sizeof(float), dpct::device_to_host);
  dpct::dpct_memcpy(h_array, d_array, 360 * sizeof(float),
                    dpct::device_to_host);
