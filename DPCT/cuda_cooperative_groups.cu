// RUN: dpct --format-range=none  -use-experimental-features=free-function-queries,logical-group -out-root %T/cuda_cooperative_groups %s --cuda-include-path="%cuda-path/include" --extra-arg="-std=c++14"
